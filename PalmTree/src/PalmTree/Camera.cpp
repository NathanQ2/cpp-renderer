...
namespace PalmTree {
    void Camera::SetOrthographicProjection(
        ...
        m_ProjectionMatrix[2][2] = 1.f / (farDist - nearDist);
        m_ProjectionMatrix[3][0] = -(right + left) / (right - left);
        m_ProjectionMatrix[3][1] = -(bottom + top) / (bottom - top);
        m_ProjectionMatrix[3][2] = -nearDist / (farDist - nearDist);
    }

    void Camera::SetPerspectiveProjection(float fovy, float aspect, float nearDist, float farDist) {
        PT_CORE_ASSERT(glm::abs(aspect - std::numeric_limits<float>::epsilon()) > 0.0f, "Invalid aspect ratio");
        const float tanHalfFovy = tan(fovy / 2.f);
        m_ProjectionMatrix = glm::mat4{0.0f};
        m_ProjectionMatrix[0][0] = 1.f / (aspect * tanHalfFovy);
        m_ProjectionMatrix[1][1] = 1.f / (tanHalfFovy);
        m_ProjectionMatrix[2][2] = farDist / (farDist - nearDist);
        m_ProjectionMatrix[2][3] = 1.f;
        m_ProjectionMatrix[3][2] = -(farDist * nearDist) / (farDist - nearDist);
    }
