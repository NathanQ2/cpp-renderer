...
namespace PalmTree {
            ...
            if (ImPlot::BeginPlot(windowName.c_str(), avail)) {
                ...
                if (ImPlot::BeginDragDropTargetPlot()) {
                    if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DATALOG_PATH_TYPE")) {
                        PlotWindowData::LineData& data = m_PlotWindowData[i].Data.emplace_back(
    ...
    float DataLoggerUI::TimestampToFloat(LogTimestamp timestamp) {
        return (float)std::chrono::duration_cast<std::chrono::milliseconds>(timestamp - m_ApplicationStartTime).count()
            / 1000.0f;
    }
    ...
    std::pair<float, float> DataLoggerUI::FindPlotMinMaxValue(const PlotWindowData& plot) {
        ...
        for (const std::vector<float>& y : plot.Data | std::ranges::views::transform(
                 [](const PlotWindowData::LineData& data) { return data.Y; }
             )) {
